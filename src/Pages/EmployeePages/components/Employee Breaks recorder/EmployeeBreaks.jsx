...
import { CircleCheckBig, Loader, PartyPopper, Smile } from "lucide-react";
import React, { useState, useEffect } from "react";
import { toast } from "react-toastify";

const EmployeeBreaks = ({ isPunchedIn }) => {
  ...
  const handleOtherBreakSubmit = async () => {
    ...
    try {
      ...
      setTodayBreaks(updatedBreaks.data.breaks?.breaks.reverse() || []);
  ...
  return (
    <div className="w-full mx-auto flex shadow-xl h-[410px]">
      {/* Left Panel - Break Controls */}
      {isLeave ? (
        <div className="w-full flex flex-col items-center justify-center text-center p-8 space-y-6 bg-gradient-to-br from-gray-50 to-gray-100 shadow-lg rounded-xl animate-fadeIn">
        {/* Heading */}
        <h3 className="text-xl font-[600] text-gray-900 tracking-tight">
          Break Tracker
        </h3>
  
        {/* Subtitle */}
        <p className="text-lg text-gray-700 font-medium">
          Enjoy your leave day! No need to worry about breaks—just relax. 🌿
        </p>
  
        {/* Icon */}
        <div className="text-rose-600 animate-pulse">
          <PartyPopper size={60} strokeWidth={1.5} />
        </div>
      </div>
      ) : !isPunchedIn ? (
        <div className="bg-white rounded-l-xl shadow-l-lg p-6 flex flex-col justify-center items-center w-full border-indigo-500">
          <h3 className="text-xl font-semibold text-gray-800 mb-4">
            Break Tracker
          </h3>
          <p className="text-gray-500 text-center">Please punch in to access break controls</p>
        </div>
      ) : (
        <div className="bg-white rounded-l-xl shadow-l-lg p-6 flex flex-row gap-6 w-full border-indigo-500">
          <div className="w-1/2">
            {/* Timer Display */}
            <h3 className="text-xl font-semibold text-gray-800 mb-1">
              Break Tracker
            </h3>
            <p className="text-gray-700 text-sm">Date: {new Date().getDate()+"-"+(new Date().getMonth()+1)+"-"+new Date().getFullYear()}</p>
            <div className="flex justify-center items-center mt-4">
              <div className="text-center bg-gradient-to-r from-indigo-50 to-blue-50 rounded-full w-40 h-40 border-2 border-indigo-200 flex flex-col justify-center items-center shadow-inner">
            ...
            <div className="grid grid-cols-2 gap-3 mt-8" id="breakSection">
              {breakOptions.map((breakOption) => {
                ...
                const breakEntry = breakHistory.find(
                  (entry) => entry.id === breakOption.id
                );
                const hasExceeded = breakEntry?.exceeded || false;
                const isDisabled =
                  breakOption.id !== "others" && isUsed && !isActive;

                return (
                  <button
                    key={breakOption.id}
                    onClick={() =>
                      handleAddBreak(breakOption.id, isUsed, isActive)
                    }
                    disabled={isDisabled}
                    className={`relative px-4 py-3 rounded-lg transition-all duration-300 ${
                      isActive
                        ? "bg-gradient-to-r from-indigo-600 to-rose-400 text-white shadow-lg transform scale-105"
          ...
          <div className="bg-white rounded-r-xl shadow-r-lg p-6 w-1/2  border-amber-500 ">
            ...
            <div className="overflow-y-auto max-h-[300px] scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-gray-50 pr-2 space-y-3">
              {todayBreaks === null ? (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div className="flex items-center space-x-2">
                          ...
                          {!break_.endTime && (
                            <span className="px-2 py-1 text-xs bg-blue-100 text-blue-600 rounded-full font-medium animate-pulse">
                              Active
                            </span>
                          )}
                        </div>
                        <span className="text-sm font-medium px-3 py-1  rounded-full shadow-sm">
                          {break_.endTime ? (
                            calculateDuration(break_.startTime, break_.endTime)
                          ) : (
                            <p className="text-sm text-red-400">In Progress</p>
                          )}
                        </span>
                            ...
                            >
                              <defs>
                                ...
                                >
                                  <stop
                                    offset="0%"
                                    style={{ stopColor: "#4f46e5" }}
                                  />
                                  <stop
                                    offset="100%"
                                    style={{ stopColor: "#fb7185" }}
                                  />
                                </linearGradient>
                              </defs>
                              <circle
                      ...
                      {break_.reason && (
                        <div className="mt-2 text-sm text-gray-600 bg-white p-2 rounded-md">
      ...
      {showReasonModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-[500px] shadow-2xl transform transition-all">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                className="px-5 py-2.5 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600 transition-colors duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
